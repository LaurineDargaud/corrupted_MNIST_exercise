...
@pytest.mark.skipif(not os.path.exists(f'{_PATH_DATA}/processed/train_dataset.pth'), reason="Data files not found")
...
def test_load_test_data():
    ...
    # check length
    # check length
    assert len(test_dataset) == 5000, "Dataset did not have the correct number of samples"
    # check shape of elements
    test_labels = []
    ...
    # check that all labels are represented
    assert list(sorted(set(test_labels))) == [i for i in range (10)], "All labels must be represented"